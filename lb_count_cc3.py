...
def get_lbs():
    ...
    try:
        r = requests.get(nw_manager_url + '/v2.0/lbaas/loadbalancers?project_id=f449ee4128c9411a8f22a736a73bf570', headers=headers)
    except requests.exceptions.RequestException as e:
        print(e)
        sys.exit(1)
    if r.status_code == 200:
        data2 = json.loads(r.text)
        lbs = data2['loadbalancers']
        x = len(lbs)
        print("Total no.of LBs created as of now = ", x)
        listeners = data['loadbalancers']['listeners']
        print("Total no.of Listeners created as of now = ", len(listeners))
...
def total_lbs():
	...
	if p.status_code == 200:
		...
		print("Total quota available for LBs =" ,data1['quota']['loadbalancer'])
