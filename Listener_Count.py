...
def get_lis():
    ...
    try:
        r = requests.get(nw_manager_url + '/v2.0/lbaas/listeners?project_id=f449ee4128c9411a8f22a736a73bf570',headers=headers)
    except requests.exceptions.RequestException as e:
        print(e)
        sys.exit(1)
    if r.status_code == 200:
        ...
        lis = data['listeners']
        x = len(lis)
        print("Total no.of Listeners created as of now = ", x)
...
def total_lis():
    ...
    if p.status_code == 200:
        ...
        print("Total quota available for LBs =", data1['quota']['listener'])
