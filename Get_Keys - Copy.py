    ...
    for j in cmp_orglst:
        ...
        for k in app_data:
            ...
            for l in range(len(consumer_data['credentials'])):
                ...
                some_dict[c] = consumer_data['credentials'][l]['consumerKey']
                some_dict[d] = consumer_data['credentials'][l]['consumerSecret']
...
def get_cc3details():
    ...
    for j in cmp_orglst:
        ...
        for k in app_data:
            ...
            consumer_data = json.loads(response1.text)
            for m in range(len(consumer_data['credentials'])):
                some_dict[a] = str(j)
                some_dict[b] = consumer_data['appId']
                some_dict[e] = consumer_data['name']
                some_dict[c] = consumer_data['credentials'][m]['consumerKey']
                some_dict[d] = consumer_data['credentials'][m]['consumerSecret']
                print(some_dict)  # Print dictionary of each app ID
                con_blst.append(some_dict.copy())
            print(con_blst)

            csv_columns = ['orgID2', 'appName2', 'appID2', 'ConKey2', 'ConSecret2']
            csv_file = "Names2.csv"
            try:
                with open(csv_file, 'w', newline='') as csvfile:
                    writer = csv.DictWriter(csvfile, fieldnames=csv_columns)
